...
const chartData = computed(() => {
  ...
  // OOM 标记：显存不足的点在左轴 y=0 处画红色 ✕
  const oomMarkers = validPoints.value.map(d => d.vramOk ? null : 0)
  ...
  return {
    ...
    datasets: [
      ...
      {
        // OOM 标记：红色 ✕ 点，始终显示在图右侧，让折线视觉上延伸到 256
        label: t('result.sweep_oom'),
        data: oomMarkers,
        yAxisID: 'yLeft',
        showLine: false,
        pointStyle: 'crossRot',
  ...
  plugins: {
    ...
    tooltip: {
      callbacks: {
        ...
        label: ctx => {
          if (ctx.datasetIndex === 2) return ` ⛔ ${t('result.sweep_oom')}`
...
<template>
  <div class="bg-white rounded-xl border border-gray-200 p-4">
    ...
    <div class="h-52 min-w-0">
      <Line v-if="sweepData?.length" :data="chartData" :options="chartOptions" />
