...
<template>
    ...
    <div class="flex items-center gap-4 min-w-0">
      <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <div class="flex flex-col">
          ...
          <span class="text-[10px] text-gray-400 leading-tight whitespace-nowrap">{{ t('nav.updated') }} 2026/04/28 17:00</span>
        </div>
        </RouterLink>
      </div>
  ...
  <nav class="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur border-t border-gray-200 flex h-14 safe-area-inset-bottom">
    ...
    >
      ...
      <span>{{ locale === 'zh' ? '支持列表' : 'Supported' }}</span>
    </RouterLink>
  </nav>
</template>
