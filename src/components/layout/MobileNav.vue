  ...
  <nav class="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200">
    <div class="grid grid-cols-4 h-16">
      ...
      >
        ...
        <span class="text-xs font-medium">{{ t('nav.estimator_short') }}</span>
      </RouterLink>
      ...
      >
        <span
          class="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full transition-colors"
          :class="isActive('/solver') ? 'bg-emerald-500' : 'bg-transparent'"
        />
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
        <span class="text-xs font-medium">{{ t('nav.solver_short') }}</span>
      </RouterLink>
      ...
      >
        <span
          class="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full transition-colors"
          :class="isActive('/library') ? 'bg-emerald-500' : 'bg-transparent'"
        />
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16" />
        </svg>
        <span class="text-xs font-medium">{{ t('nav.library_short') }}</span>
      </RouterLink>
    </div>
  </nav>
</template>
