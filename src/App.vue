...
// 切换模型时，ctx 更新为 min(max_ctx, 16384)，prompt/output 保持用户设置不变
watch(model, (m) => {
  if (!m?.max_ctx) return
    ...
    <TwoColumn>
      <template #config>
        ...
        <RunConfig
          ...
          v-model:ctx="ctx"
          v-model:batch="batch"
          v-model:promptLen="promptLen"
          v-model:outputLen="outputLen"
          v-model:flashAttention="flashAttention"
          v-model:kvCacheQuant="kvCacheQuant"
          v-model:prefixCacheHit="prefixCacheHit"
          v-model:cpuOffload="cpuOffload"
          v-model:pcieBw="pcieBw"
          :model="model"
        />
      </template>
      <template #result>
